...
module.exports = {
  ...
  async execute(message, args) {
    ...
    const targetDisplayName = args[1] ? args.slice(1).join(" ") : message.member.displayName;

    if (!teamName || (teamName !== 'blue' && teamName !== 'red')) {
      return message.channel.send("이동할 팀을 지정해주세요. 예: `!팀이동 blue` 또는 `!팀이동 red [닉네임]`");
    }
    ...
    if (index === -1 && teamDestination.includes(targetDisplayName)) {
      return message.channel.send(`${targetDisplayName}님은 이미 ${teamName.toUpperCase()} 팀에 있습니다.`);
    } else if (index === -1 && !serverState.participants.includes(targetDisplayName)) {
      return message.channel.send(`${targetDisplayName}님은 현재 게임에 참가하지 않았습니다.`);
    }
    ...
    // 팀 이동 결과 출력
    await message.channel.send(`${targetDisplayName}님이 ${teamName.toUpperCase()} 팀으로 이동했습니다.`);
